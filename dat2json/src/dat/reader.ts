...
export function readInteger(
  ...
  if (size === 1 && unsigned) return (offset) => data.getUint8(offset);
    ...
    return (offset) => Number(data.getBigUint64(offset));
    ...
    return (offset) => Number(data.getBigInt64(offset));
...
export function readKeySelf(
  data: BinaryReader
): (offset: number) => DatKeySelf {
...
export function readKeyForeign(
  data: BinaryReader
): (offset: number) => DatKeyForeign {
  const NULL = getNULL(data.ptrsize);
  return (offset) => {
    const rowIdx = data.getSizeT(offset);
    return rowIdx === NULL ? null : rowIdx;
...
export function getArrayReader(header: Header, datFile: DatFile) {
  ...
  const [elSize, reader] = (() => {
    if (type.boolean) {
      return [1, (offset: number) => Boolean(dataVariable[offset])] as const;
    }
    if (type.string) {
      const readString_ = readString(dataVariable);
      return [
    ...
    if (type.key) {
      if (type.key.foreign) {
        ...
        return [datFile.fieldSize.KEY, readKeySelf(readerVariable)] as const;
      }
    }
