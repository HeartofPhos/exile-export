...
const oneBool: One<boolean> = (reader, offset) =>
  Boolean(reader.getUint8(offset));
const oneUint1: One<number> = (reader, offset) => reader.getUint8(offset);
const oneUint2: One<number> = (reader, offset) =>
  reader.getUint16(offset, true);
const oneUint4: One<number> = (reader, offset) =>
  reader.getUint32(offset, true);
const oneUint8: One<number> = (reader, offset) =>
  Number(reader.getBigUint64(offset, true));
...
const oneInt8: One<number> = (reader, offset) =>
  Number(reader.getBigInt64(offset, true));
const oneDecimal4: One<number> = (reader, offset) =>
  reader.getFloat32(offset, true);
const oneDecimal8: One<number> = (reader, offset) =>
  reader.getFloat64(offset, true);
...
export function getFieldReader(header: Header, datFile: DatFile) {
  ...
  if (type.boolean) {
    oneFn = oneBool;
    elSize = fieldSize.BOOL;
  } else if (type.string) {
    oneFn = oneString;
    elSize = fieldSize.STRING;
  } else if (type.integer) {
    const { size, unsigned } = type.integer;
    elSize = size;
    if (size === 1 && unsigned) {
      oneFn = oneUint1;
    } else if (size === 2 && unsigned) {
      oneFn = oneUint2;
    } else if (size === 4 && unsigned) {
      oneFn = oneUint4;
    } else if (size === 8 && unsigned) {
      oneFn = oneUint8;
    } else if (size === 1 && !unsigned) {
      oneFn = oneInt1;
    } else if (size === 2 && !unsigned) {
      oneFn = oneInt2;
    } else if (size === 4 && !unsigned) {
